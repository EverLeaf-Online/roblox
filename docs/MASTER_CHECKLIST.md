...
- [ ] REBUILD production world-art pipeline around curated free Creator Store 3D kits plus strict EverLeaf sanitization/layout/collision; the generated Blender architecture is retired.
...
- [ ] **PARTIAL:** curated free Creator Store architecture is now allowed only through the sandboxed/sanitized production registry; random Marketplace/Toolbox insertion remains prohibited; final acceptance requires the world to actually look and play at the intended quality bar in Studio.
- [x] **V11 collision architecture source-complete:** Lumenreach visual meshes/parts are render-only by default; buildings, props, landmarks, bridges, docks, fences, imported solid dressing, and gameplay workstations use explicit simplified collision proxies that are also queryable by the scripted camera/LOS systems. Studio traversal QA remains required before collision is accepted.
- [x] **V14 frontier architecture conversion:** removed the obsolete generic `makeBuilding` shell; Wayfarer Camp, Greenway/Crossroads, Glowmere, Eastbridge, Mossglen, Sunmoss, Veilfall, Proving support buildings, hamlet cottages, patrol posts, and wayhouse micro-sites now route through explicit purpose-specific architecture archetypes. Studio visual acceptance remains required.
- [x] **V15 architecture correction:** V14 still read too blocky in Studio feedback, so core frontier archetypes gained visible timber trusses/gable framing, door hoods, facade depth, chimneys and purpose-specific silhouette elements; Wayfarer Camp is now graded before placement and core buildings face the plaza/street hierarchy. Studio acceptance remains pending.
- [x] **V16 real mesh architecture foundation:** created eight original EverLeaf low-poly GLB buildings in Blender, uploaded all eight as approved Roblox model assets, registered them for sanitized runtime loading, and replaced the civic hall/depot/archive/inn/healer/workshop/stable/proving visible shells with MeshPart prefabs plus simple explicit collision proxies. Studio visual acceptance remains pending.
- [x] **V17 district mesh architecture batch:** added eight more approved original EverLeaf GLBs for road wayhouses, barns, two croft variants, Glowmere stilt housing, Mossglen ranger halls, frontier gatehouses, and ruined halls; these builders now place MeshPart prefabs with simplified collision instead of constructing the visible building from Parts. Studio visual acceptance remains pending.
- [x] **V18 landmark mesh architecture batch:** converted Wayfarer forge yards, Gravebone mausoleums, Sunmoss observatory, Veilfall sanctuary, Shattered Arch fortress towers, and stone shrines to approved original EverLeaf GLBs with render-only visuals and explicit simple collision proxies. Studio visual acceptance remains pending.
- [x] **V19 Studio screenshot correction:** all 22 Lumenreach architecture GLBs were regenerated/re-uploaded with an embedded color-atlas texture to prevent pale/white material loss; architecture visuals now participate in camera/LOS queries without becoming physical, and collision proxies scale with the placed mesh. Fresh Studio verification remains pending.
- [x] **V20 Wayfarer MMO-hub composition pass:** tightened the starter town into a civic square, market lane, service/production courts, defended gate thresholds, stronger street furniture/signage, and a denser ambient resident population instead of a sparse building ring around open ground. Studio acceptance remains pending.
- [x] **Whole-world collision/water source audit:** Brasshaven/Belforge now use the same explicit proxy-only collision contract for ground, boundaries, structures, encounter machinery, gates, arena/antechamber geometry, and vendor counters; all current camera/grounding/spawn/targeting/projectile raycasts ignore traversable Terrain water while Roblox swimming remains untouched.
- [x] First functional world-activity pass: gatherable Lumenleaf/Glowreed/Sunmoss/Veilwater nodes, persistent Wayfarer caches, rest shrines, an apothecary crafting bench, four additional quest NPCs, and four optional regional side quests are wired server-authoritatively.
...
- [x] Terrain and native structures now carry the macro environment; approved TreePrimary instances are used sparingly for zone-edge forest massing, while unstable RockFace and scenery-stack dependencies are excluded from the V3 layout.
- [x] Oversized/unintegrated cliff meshes and black stacked-rock formations are removed from the canonical map path; cliffs, ridges, gorge walls, highlands, and ravines are Terrain-authored.
- [x] Wayfarer Camp is rebuilt as a permanent town with an open plaza, Wayfarer Hall, Quartermaster House, Archive Lodge, Wayfarer Inn, training yard, departure gate, and no tent-based town composition.
...
- [x] Lumenreach NPCs use grounded R15 humanoid rigs with distinct role silhouettes and terrain-only foot grounding; Ilyra, Orin, Maela, and Seren remain in Wayfarer Camp while Tovin is staged at the Eastbridge Landing field outpost; Studio visual QA remains.
...
- [x] V3 density is intentionally restrained: route-aware tree massing frames zones and roads while encounter centers, town circulation, and action-combat lanes remain open.
- [x] Lumenreach V3 macro-layout replacement is source-complete: the generator clears old Terrain/content, rebuilds the region from scratch, carves the gorge after roads, uses Eastbridge as the main threshold and Reedbridge as a secondary loop, raises Sunmoss as a true shelf, lowers Veilfall as a true ravine, and converges late routes at Shattered Lumen Arch; fresh Studio visual QA remains.
- [x] Terrain-room construction is source-complete for Camp, Greenway, Crossroads, Glowmere, Eastbridge Landing, Gravebone, Mossglen/Bramble, Sunmoss, Veilfall, Ruins, and Proving; mountain belts and internal ridges communicate boundaries without a visible invisible-wall ring.
...
- [x] Lumenreach V11 collision policy supersedes the V10 name-whitelist approach: all rendered world geometry is non-colliding by default, every intended solid uses an explicit queryable proxy, imported trees align collision to the actual trunk, and solid rocks/logs/stumps/roots receive simplified proxies. Studio walk-through QA remains.
- [x] Lumenreach V13 screenshot-driven correction: building roof collision is trimmed to the wall footprint so eaves no longer create shoulder-height barriers, forge awning clearance was raised and made render-only, floating-looking buildings use deeper buried foundations, overlapping duplicate ambient NPCs were removed, and 20 additional authored micro-sites populate inter-zone gaps. Studio visual/traversal QA remains.
- [ ] **WHOLE-GAME ARCHITECTURE OVERHAUL ACTIVE:** replace generic block/gable building shells across Lumenreach, Brasshaven/Eastworks, and Belforge with purpose-built regional archetypes. Canonical production checklist: `docs/ARCHITECTURE_OVERHAUL_CHECKLIST.md`.
...
- [x] Regional NPC services beyond Ilyra are implemented: Orin and Maela remain in Wayfarer Camp, while Tovin/Scout Outfitter is stationed at Eastbridge Landing as the eastern field quest hub.
...
- [x] Landmark exploration triggers are wired to the V3 geography: Crossroads, Glowmere, Eastbridge, Gravebone Watch, Mossglen, Proving Circle, Veilfall, Sunmoss, and Shattered Lumen Arch.
...
- [ ] Re-author an optional hidden V3 story/cache location after terrain/traversal QA; the legacy Whisperroot placement was deliberately removed with the replaced map.
...
- [x] Lumenreach exploration quest chain aligned to the V3 route network and its active landmarks.
...
- [x] Lumenwood Crossroads is the western navigation hub leading toward Glowmere and Eastbridge.
- [x] A terrain-carved river/gorge divides the western and eastern progression halves; roads are cut first and the gorge is carved afterward so accidental land bridges cannot form.
- [x] **Eastbridge** is the main progression crossing; **Reedbridge** is the secondary Glowmere→Mossglen loop crossing.
- [x] **Eastbridge Landing** is a real Level 3–7 field outpost with Tovin/Scout Outfitter, reducing MMO quest backtracking between Camp and the eastern hunt zones.
- [x] Glowmere is a true Terrain wetland basin with a water body, shoreline combat/exploration space, and alternate northern route into Mossglen.
- [x] Gravebone Watch is the Level 3–7 ruined-combat branch after Eastbridge, hosting Skeletons and the Gravebone Captain advancement target.
- [x] Mossglen Valley is the primary Level 4–7 party field with Ridgebeaks, Brambleback side branch, and a dedicated Mosswarden field-boss arena.
- [x] Sunmoss Ridge is a genuine elevated Terrain shelf rather than decorative rocks stacked around a flat floor.
- [x] Veilfall Ravine is a genuine lowered Terrain room connected from Gravebone and returning toward the Shattered Arch; the waterfall uses stable Beam ribbons and Terrain cliff shoulders.
- [x] Shattered Lumen Arch is the far-east Level 8–10 capstone and Lumenreach→Brasshaven portal destination.
- [x] Brasshaven→Lumenreach return travel now targets a dedicated eastern gate spawn instead of teleporting established players back to the starter plaza.
- [x] Wayfarer Proving Circle remains detached south of town so first advancement is a deliberate town-return milestone rather than route clutter.
- [x] Primary roads are terrain-authored MMO lanes approximately 15–22 studs wide with party/dodge clearance; encounter centers are not used as roads.
- [x] Outer map limits use mountain Terrain belts and internal ridge massing instead of a visible ring of arbitrary invisible walls.
- [x] Tree placement is authored as perimeter/zone-edge massing; approved tree visuals are render-only with predictable trunk collision and do not randomly fill roads or encounter centers.
...
- [ ] **Studio streaming/performance pass:** Camp, Glowmere, Mossglen, Sunmoss, Veilfall, and Shattered Arch worst-case streaming, instance count, terrain memory, and frame time.
...
4. [ ] Verify visible loot pickup, Collect objectives, equipment rewards, Lumenreach + Brasshaven shops, and regional/Belforge travel across death/rejoin.
...
- [x] **V22 actual building-layout pass:** removed the V21 clutter-court overlays instead of stacking more props; re-laid the real buildings across all major Lumenreach districts and secondary hamlet/patrol/ruin archetypes around streets, compounds, boardwalks, fort courts and approach axes. Added eight original approved Brasshaven industrial MeshPart buildings and arranged them into real industrial blocks across Foundry Threshold, Gearworks, Cinder, Sootworks, Eastworks, Blueglass, Authority/Flux, Runoff, Bellows and Belforge approach. Studio visual acceptance remains pending.
...
- [x] **2026-09-16 Creator Store architecture reset:** retired the malformed/generic generated Lumenreach and Brasshaven building kits from active registry use; all 30 logical architecture roles now point at curated free Creator Store models, third-party free loading is enabled through `AssetService:LoadAssetAsync()`, executable/behavioral descendants are stripped, provenance/credit metadata is recorded, and EverLeaf-authored collision/layout authority remains intact. Studio player-height replacement review is still required.
...
- 2026-09-16 Creator Store import correction: strip embedded UI/messages plus lights/VFX/behavior, discard imported/arbitrary PrimaryPart pivots (which could tilt whole buildings), neutralize model pivot rotation, and clamp architecture normalization to 0.55x–1.75x. Brasshaven V6 uses these guards across every curated building.

- 2026-09-16 **Lumenreach V28 cohesive Creator Store correction:** Studio screenshots proved the mixed one-off free-model library was visually unacceptable (inconsistent styles, malformed/partial buildings, giant helper geometry, bad pivots and pack-level imports). Replaced all 22 Lumenreach architecture roles with two cohesive free source packs, added role-aware submodel extraction and per-asset caching, fail-closed whole-pack rejection, invisible/giant-helper stripping, visible-bounds recentering/grounding, and tighter 0.55x–1.75x scale normalization. Fresh Studio review is required before acceptance.
